...
describe("AnalyticsHomePage", () => {
  ...
  it("renders each chart with an accessible summary", async () => {
    ...
    expect(
      screen.getByRole("img", { name: /Monthly signups/i }),
    ).toBeInTheDocument();
    expect(
  ...
  it("builds the profession by country matrix", async () => {
    ...
    expect(within(canadaRow).getByRole("rowheader")).toHaveTextContent(
      "Canada",
    );
    expect(within(canadaRow).getAllByRole("cell").at(-1)).toHaveTextContent(
      "6",
    );
  });
